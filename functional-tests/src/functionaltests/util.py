...
class Base(TestCase):
    ...
    def setUp(self):
        execute_http(
            blankpage_v1_method("Purge"),
            body=blankpagev1.PurgeRequest(),
        )
        execute_http(
...
@cache
def blankpage_service():
    return os.environ.get("BLANKPAGE_URL", "http://localhost:8080")


def blankpage_v1_method(method):
    return f"{blankpage_service()}/blankpage.v1.BlankPageService/{method}"


def pauth_v1beta1_method(method):
    return f"{blankpage_service()}/pauth.v1beta1.PAuthService/{method}"
