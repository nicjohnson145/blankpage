	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return withLogging(cmd, func(logger zerolog.Logger) error {
				return withBlankpageClient(func(client pbv1connect.BlankPageServiceClient) error {
					...
					if cmd.Flags().Lookup(flagTitle).Changed {
						resolvedTitle = overrideTitle
					} else if resp.Msg.Metadata.Title != "" {
						resolvedTitle = resp.Msg.Metadata.Title
					}

					if cmd.Flags().Lookup(flagAuthor).Changed {
						resolvedAuthor = overrideAuthor
					} else if resp.Msg.Metadata.Author != nil {
						resolvedAuthor = *resp.Msg.Metadata.Author
					}

					if cmd.Flags().Lookup(flagSeries).Changed {
						resolvedSeries = overrideSeries
					} else if resp.Msg.Metadata.Series != nil {
						resolvedSeries = *resp.Msg.Metadata.Series
					}

					if cmd.Flags().Lookup(flagSeriesNum).Changed {
						resolvedSeriesNum = fmt.Sprint(overrideSeriesNumber)
					} else if resp.Msg.Metadata.SeriesNumber != nil {
